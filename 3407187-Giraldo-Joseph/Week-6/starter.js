...
const users = [
  ...
  { name: "Ana Gómez", category: "Desarrollador Frontend", value: 4500 },
  { name: "Carlos Pérez", category: "Analista de Recursos Humanos", value: 3800 },
  { name: "Laura Rodríguez", category: "Gerente de Proyecto", value: 6000 },
  { name: "Miguel Torres", category: "Desarrollador Backend", value: 4700 },
  { name: "Sofía Martínez", category: "Diseñador UX/UI", value: 4200 },
  { name: "Andrés López", category: "Especialista en Reclutamiento", value: 3900 }
...
const valueLabel = "Salario"; // ← cambiar
...
for (const user of users) {
  ...
  console.log(`${lineNumber}. ${user.name} - ${user.category} - ${valueLabel}: ${user.value}`);
}
...
for (const user of users )
if (user.category === category) count ++;
...
const averageValue = users.length > 0 ? totalValue / users.length : 0;
...
let maxItem = users[0] ?? null;
let minItem = users[0] ?? null;

if (users.length > 0) {
  // TODO: Recorre con for...of y compara values para encontrar max y min
  for (const user of users) {
    if (user.value > maxItem.value)
        maxItem = users;
    if (user.value <minItem.value)
        minItem = users;
...
const comparison = user.value > averageValue ? "sobre el promedio" : "bajo el promedio";
  // TODO: Imprime la línea del reporte
  console.log(`${i + 1}. ${user.name} — ${comparison}`);
}
